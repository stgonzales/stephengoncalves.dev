...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`CREATE TABLE \`defaults_socila_links_handlers\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`name\` text NOT NULL,
  ...
  await db.run(sql`CREATE TABLE \`defaults\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`personal_details_name\` text NOT NULL,
  	\`personal_details_role\` text NOT NULL,
  	\`personal_details_location\` text NOT NULL,
  	\`personal_details_email\` text NOT NULL,
